...
// NOTE: Since this is an array, registered users will be reset when server restarts.
...
// 2. UNIFIED AUTHENTICATION ROUTE (LOGIN)
...
// 2.5 REGISTRATION ROUTE (NEW: Handles /auth/register)
app.post('/auth/register', (req, res) => {
    // We expect the form to have fields: 'username' (for ID), 'password', and 'fullname'
    ...
    const exists = registeredStudents.find(s => s.id === newId);
    if (exists) {
        return res.redirect('/register.html?error=user_already_exists');
    }

    // Add to Mock Database (InMemory)
    ...
    console.log(`[REGISTER] New Student Registered: ${newName} (${newId})`);
