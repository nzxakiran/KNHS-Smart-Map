...
app.post('/api/sync', async (req, res) => {
    ...
    // 2. NEW: Send Full History Report to Admin
    // This allows the admin to see the "Story" of what happened while offline
...
io.on('connection', async (socket) => {
    ...
    socket.on('student-note', (data) => { socket.join(data.id); io.emit('admin-receive-note', data); });
    socket.on('student-status', (data) => { if(data.id) socket.join(data.id); io.emit('admin-dashboard-update', data); });
    socket.on('admin-reply', (data) => { io.to(data.targetId).emit('receive-reply', { message: data.message }); });
});
